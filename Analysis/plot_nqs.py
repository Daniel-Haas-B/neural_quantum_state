...
def plot_part1(filename="Part1", particles=1, dims=1, interacting=False, save=False):
    ...
    # plot energy
    ax1 = sns.lineplot(data=df_detailed_2hidden, x="Epoch", y="Energy", hue="LearningRate", legend=True, palette="pastel", ax=ax1)
    ...
    for i, lr in enumerate(lrs):
        ...
        E, E_std, epoch, MCCs = df_lr["Energy"].to_numpy(), df_lr["Energy_std"].to_numpy(), df_lr["Epoch"].to_numpy(), df_lr["Metro-steps"].to_numpy()
        E_std = np.sqrt(E_std)/(MCCs)
        # fill between
        ax1.fill_between(epoch, E - 2*E_std, E + 2*E_std, color=cmap(Ds[0]), alpha=0.2)

    df_detailed_2hidden["Energy_std"] = df_detailed_2hidden["Energy_std"]/df_detailed_2hidden["Metro-steps"]

    # plot variance
    ax2 = sns.lineplot(data=df_detailed_2hidden, x="Epoch", y="Energy_std", hue="LearningRate", legend=True, palette="pastel", ax=ax2)
    ...
    ax1.axhline(y=analytical_solutions[0],color=cmap(Ds[0]), linestyle='--', label="Analytical solution")

    # add same legend for both plots, in between them
    handles, labels = ax2.get_legend_handles_labels()
    # add legend on top of the plot
    ax2.legend(handles=handles, labels=labels, title="Learning rate", fontsize=11, loc="upper center", bbox_to_anchor=(0.5, 2.4), ncol=4)
    ...
    print(f"Energy closest to analytical solution: {df_detailed['Energy'].iloc[idx]}")

    df_detailed["transformEnergy"] = np.log(np.abs(df_detailed["Energy"] - analytical_solutions[0]))

    # plot the heatmap
    ax = sns.heatmap(df_detailed.pivot("Hidden-nodes", "LearningRate", "transformEnergy"), annot=True, fmt=".2f", linewidths=.5, cmap="Purples")
    # lable the colorbar
    ...
    print(f"Energy closest to analytical solution: {df_detailed[['Energy', 'Metro-steps']].iloc[np.argmin(np.abs(df_detailed['Energy'] - analytical_solutions[0]))]}")

    df_detailed["Energy"] = np.abs(df_detailed["Energy"] - analytical_solutions[0])
    
    ax = sns.lineplot(data=df_detailed, x="Metro-steps", y="Energy", hue="LearningRate", legend=True, palette="pastel")
    ax.get_legend().remove()
    ax.set(xlabel=r"Metro-steps", ylabel=r"$\log(|\langle E_L \rangle - E_{exact}|)$")
    ...
    ax.legend(handles=handles, labels=labels, title="Learning rate", fontsize=11)
...
def plot_part2(filename="Part2", save=False, interacting=False):
    ...
    if not cpp_utils.dataPath(filename + ".txt").exists():
        for i, importance in enumerate(importances):
            for j, stepLength in enumerate(stepLengths):
                cpp_utils.nqsRun(
                    D=D, N=N, hiddenNodes=hiddenNodes, logMet=logMet, logEq=logEq, 
                    stepLength=stepLength, importance=importance, optimizerType=optimizerType, 
                    learnRate=lr, interacting=interacting, filename=filename, detailed=False
                )
    ...
    print(df_detailed[df_detailed.Epoch == 99]["Imposampling"], 
    ...
    df_detailed = df_detailed[(df_detailed.Epoch > 0) & (df_detailed.Epoch < 30)]

    # change Imposampling column to be "Metropolis" and "Metropolis-Hastings"
    df_detailed["Imposampling"] = df_detailed["Imposampling"].replace({0: "Metropolis", 1: "Metropolis-Hastings"})

    # to show that the importance sampling is better, we plot the energy per epoch for both cases
    df_detailed["transformEnergy"] = np.log(np.abs(df_detailed["Energy"] - analytical_solution))
    # take the moving average of the energy per epoch, to smooth the plot with a window of 2
    df_detailed["transformEnergy"] = df_detailed["transformEnergy"].rolling(window=4).mean()

    ax = sns.lineplot(data=df_detailed, x="Epoch", y="transformEnergy", hue="StepLength", style="Imposampling", legend=True, palette="pastel")
    ax.get_legend().remove()
    ax.set(xlabel=r"Epoch", ylabel=r"$\log(|\langle E_L \rangle - E_{exact}|)$")
    ...
    ax = sns.lineplot(data=df_detailed, x="Epoch", y="Accept_ratio", hue="StepLength", style="Imposampling", legend=True, palette="pastel")
    ax.get_legend().remove()
...
def plot_part3(filename="Part3" ,save=False):
    ...
    N = 2
    hiddenNodes = np.array([2]) # if you loose the nice plot, turn this back to 2
    D = 2
    ...
    #start time measurement
    start = time.time()
    filename += f"_N={N}_D={D}_interacting={interacting}_importance={importance}"
    # if not cpp_utils.dataPath(filename + ".txt").exists():
    ...
    ax = sns.lineplot(data=df_detailed, x="Epoch", y="Energy", hue="optimizerType", legend=True, palette="pastel")
    ...
    # ## add analytical solution
    ax.axhline(y=analytical_solution, color=cmap(D), linestyle='--', label=f"Analytical")

    # ## add aesthetic error region
    for i, opt in enumerate(optimizerTypes):
        df_opt = df_detailed[df_detailed.optimizerType == opt]
        E, E_std, epoch, MCCs = df_opt["Energy"].to_numpy(), df_opt["Energy_std"].to_numpy(), df_opt["Epoch"].to_numpy(), df_opt["Metro-steps"].to_numpy()
        E_std = np.sqrt(E_std)/(MCCs)
        # fill between
        ax.fill_between(epoch, E - 2*E_std, E + 2*E_std, alpha=0.2, color=cmap(i))
...
def plot_part3_adam_investigation(filename="Part3_adam_investigation" ,save=False):
    ...
    df_concat["epsilon"] = df_concat["epsilon"].apply(lambda x: np.abs(np.log10(x)/10))
    ...
    # rename columns
    df_concat = df_concat.rename(columns={"beta1": r"$\beta_1$", "beta2": r"$\beta_2$", "epsilon": r"$|\log_{10}(\epsilon)|/10$"})


    # create normalized colormap
    norm = mpl.colors.Normalize(vmin=df_concat["Energy"].min(), vmax=df_concat["Energy"].max())

    cmap = mpl.cm.ScalarMappable(norm=norm, cmap="magma_r")
    cmap.set_array([])

    pd.plotting.parallel_coordinates(df_concat, "Energy", color=cmap.to_rgba(df_concat["Energy"]))
    # make colorbar on bottom

    plt.gca().lines[0].set_linewidth(5) # lines[0] is the first line, lines[1] is the second line, etc
...
####################################
def plot_optimal_energy_per_epoch(filename="adam_opt_GD_energy_per_epoch", D=2, interacting=True, importance=True, save=False):
    Ns = np.array([2])
    ...
    ax.fill_between(df_detailed["Epoch"], df_detailed["Energy"] - df_detailed["Energy_std"]/np.sqrt(df_detailed["Metro-steps"]), df_detailed["Energy"] + df_detailed["Energy_std"]/np.sqrt(df_detailed["Metro-steps"]), alpha=0.2, color="purple")
    ...
    handles, labels = ax.get_legend_handles_labels()
    labels = ["Analytical", "Energy optimal model", r"$\pm \sigma/\sqrt{MCCs}$"]
    handles = [mpl.lines.Line2D([], [], color=cmap(D), linestyle='--', label="Analytical"),
                 mpl.lines.Line2D([], [], label="Energy optimal model"), mpl.patches.Patch(facecolor="purple", alpha=0.2, label=r"$\pm \sigma/\sqrt{M}$")]
    ax.legend(handles=handles, labels=labels, title="", fontsize=11)
    ...
    df_detailed["Energy_std"] = df_detailed["Energy_std"]/df_detailed["Metro-steps"]
    # eliminate largest value 
    df_detailed = df_detailed[df_detailed.Energy_std != df_detailed.Energy_std.max()]
    ...
    df_gradNorm = pd.concat([df_HiddenGradNorm, df_VisibleGradNorm, df_WeightGradNorm])
    # take moving average of energy per epoch, window of 10
    df_gradNorm["HiddenGradNorm"] = df_gradNorm["HiddenGradNorm"].rolling(window=10).mean()

    # print energy per epoch and use grad as hue
    ax = sns.lineplot(data=df_gradNorm, x="Epoch", y="HiddenGradNorm", hue="grad", legend=True, palette="pastel")
...
if __name__ == "__main__":
    ...
    #### maybe just maybe show the difference in timing of PCG vs MT
    ...
    #plot_energy_per_epoch(filename="banan", D=2, interacting=True, optimizerType = "vanillaGD", importance=True, save=False)
    ...
    # # Part 4 now with interaction, investigate again the learning rate and number of hidden nodes. Here probably we need to do a grid search
    #plot_optimal_energy_per_epoch(filename="Part4", D=2, interacting=True, importance=True, save=True)

    # # Pasrt 5: use netket versus us: both vmc and nqs
    # plot_energy_per_epoch(filename="Part4.2", D=2, interacting=True, optimizerType = "All", importance=False, save=False)
