...
def nqsRun(D=2, N=2, hiddenNodes=2,  logMet=20, logEq=16, stepLength=0.6, importance=False, optimizerType="vanillaGD", learnRate=0.1 ,interacting=False, filename="test.txt", detailed=False):
    nqs_path = nqsPath()
...
def nqsRunAdamopt(hiddenNodes=2, logMet=14, logEq=14, importance=True, beta1=0.9, beta2=0.99, epsilon=1e-8,
                learnRate=0.1, interacting=True, filename="adam_investigation", detailed=False):
    nqsadam_path = nqsPath()
    
    filename_path = dataPath(filename)

    assert nqsadam_path.exists(), f"I cannot find {nqsadam_path} :((, are you sure you have compiled?"
    args = [
        nqsadam_path,
        2,
        2,
...
def nqsLoad(filename):
    ...
    int_cols = ["Dimensions", "Particles", "Hidden-nodes","Metro-steps", "Imposampling", "Epoch", "Interaction"]
